...
for config_file in glob.glob(args.folder + "/**/config.json"):
    ...
    for invocation in config:
        ...
        for i in range(args.times):
            ...
            if method in ["qcqp", "pso", "qcqp-incremental"]:
                command = "cd {prophesy_dir}".format(
                    pipenv_dir = args.pipenv_dir,
                    prophesy_dir = args.prophesy_dir,
                )
                prophesy_prop_file_name = ".".join(drn_file.split(".")[:-1]) + "_prophesy.prop"
                with open(prophesy_prop_file_name, "w") as f:
                    f.write(prophesy_prop)
                prophesy_command = "time python3 -m pipenv run python scripts/parameter_synthesis.py load-problem {file} \"{prophesy_prop}\" set-threshold \"{prophesy_threshold}\" find-feasible-instantiation --qcqp-handle-violation minimisation --qcqp-mc full --precheck-welldefinedness {incremental_flags} {prophesy_dir} {method}".format(
                    prophesy_prop = prophesy_prop_file_name,
                    file = drn_file,
                    prophesy_threshold = str(prop_dict["bound"]),
                    prophesy_dir = "above" if prop_dict["dir"] == "max" else "below",
                    incremental_flags = "--qcqp-incremental --qcqp-store-quadratic" if method == "qcqp-incremental" else "",
                    method = method.replace("-incremental", ""),
                )
                command += "\n" + prophesy_command
            else:
                command = "time {binary} {command} --explicit-drn {file} --prop \"{property}\" {method} {exact} --descent-method {descent_method} {additional_storm_args}".format(
                    command = "--gradient-descent" if prop_dict["bound"] else "--find-extremum",
                    binary = args.storm_pars,
                    file = drn_file,
                    property = escaped_prop,
                    method = "--core:eqsolver gmm++ --learning-rate " + str(invocation["learning_rate"]),
                    exact = "--exact" if args.exact else "",
                    descent_method = invocation["method"],
